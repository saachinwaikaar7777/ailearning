...
# Fix Windows console encoding for Unicode output (e.g. emoji like checkmark)
...
# Import the Google GenAI SDK (uses Google API Key, not OpenAI)
import  google.generativeai as genai

load_dotenv()
# Read Google API key from .env (handles different casings)
...
EMBED_MODEL = "models/gemini-embedding-2"   # Google Gemini embedding model
...
def embed_texts(texts: List[str]) -> List[List[float]]:
    """
    Embed a list of texts using Google Gemini's embedding model.
    Returns a list of embedding vectors (lists of floats).
    """
    return [
        genai.embed_content(model="models/gemini-embedding-001", content=text)["embedding"]
        for text in texts
    ]
